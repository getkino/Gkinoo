...
export async function getTmdbMovieInfo(query) {
  ...
  try {
    const res = await fetch(`${BASE_URL}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}`);
    const data = await res.json();
    const first = data?.results?.[0];
    if (first) {
      // Detayları çek
      const detailsRes = await fetch(`${DETAILS_URL}${first.id}?api_key=${TMDB_API_KEY}&language=tr-TR&append_to_response=watch/providers`);
