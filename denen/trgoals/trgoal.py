...
class TRGoals:
    ...
    def referer_domainini_al(self):
        ...
        if eslesme := re.search(referer_deseni, icerik):
            return eslesme[1].strip("/").lower()
        else:
            raise ValueError("M3U dosyasında 'trgoals' içeren referer domain bulunamadı! "
                             "Örnek: #EXTVLCOPT:http-referrer=https://trgoals123.xyz")
    ...
    def redirect_gec(self, redirect_url: str):
        ...
        try:
            response = self.httpx.get(redirect_url, headers=self.headers, follow_redirects=True)
        except Exception as e:
    ...
    def yeni_domaini_al(self, eldeki_domain: str) -> str:
        ...
        try:
            ...
            konsol.log("[red][!] redirect_gec(eldeki_domain) hatası.")
            try:
                yeni_domain = check_domain(self.trgoals_domaini_al())
            except Exception:
                konsol.log("[red][!] trgoals_domaini_al hatası.")
                try:
                    yeni_domain = check_domain(self.redirect_gec("https://t.co/MTLoNVkGQN"))
                except Exception:
                    konsol.log("[red][!] Yedek fallback başarısız.")
    ...
    def m3u_guncelle(self):
        ...
        yedek_dosya = self.m3u_dosyasi.replace(".m3u", "_backup.m3u")
        with open(yedek_dosya, "w") as yedek:
            yedek.write(m3u_icerik)
            konsol.log(f"[blue][~] Yedek dosya oluşturuldu: {yedek_dosya}")
        ...
        response = self.httpx.get(kontrol_url, headers=self.headers, follow_redirects=True)
        ...
        konsol.log("[bold green][✔] Güncelleme başarıyla tamamlandı!")

if __name__ == "__main__":
    guncelleyici = TRGoals("C:/Users/dlmhak.MPH/Desktop/denen/trgoals/trgoals.m3u")
